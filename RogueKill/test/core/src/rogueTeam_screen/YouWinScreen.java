...
public class YouWinScreen extends BaseScreen {
	...
	private Sound youWin = Gdx.audio.newSound(Gdx.files.internal("YouWin.wav"));
	private boolean flag = false;

	public YouWinScreen(MyGame g, MainMenuScreen _ms, String PlayerName, Integer PlayerScore) {
		...
		Label titolo = new Label("HAI VINTO", creditsStyle);
	...
	public void render(float delta) {
		...
		if (Gdx.input.isKeyJustPressed(Keys.ESCAPE)) {
			Score s = new Score(PlayerName, PlayerScore + 1000);//AGGIUNGO 1000 PUNTI PER COMPLETAMENTO DELLA PARTITA
			s.createUser();

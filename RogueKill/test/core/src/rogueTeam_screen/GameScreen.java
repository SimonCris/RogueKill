...
public class GameScreen extends BaseScreen {
	...
	public GameScreen(MyGame g, MainMenuScreen _ms, String s) {
	...
	public GameScreen(MyGame g, PauseMenuScreen pm) {
	...
	public void create() {
		...
		setMap(new MapCreator(47));
		wallList = map.getWallList();// setto la lista dei muri con i muri
										// creati con mapCreator
		proiettili = map.getProiettili();// setto la lista di proiettili del
											// gameScreen alla lista di
											// mapCreator
		lifePackList = map.getLifePackList();// setto la lista di medikit del
												// gameScreen alla lista di
												// mapCreator
		ammoList = map.getAmmoList();// setto la lista di munizioni del
										// gameScreen alla lista di mapCreator
		sSoldierList = map.getsSoldierList();// setto la lista di soldati del
												// gameScreen alla lista di
												// mapCreator
	...
	public void render(float delta) {
		...
		if (Gdx.input.isKeyJustPressed(Keys.ESCAPE)) {
			...
			game.pause();
			pm = new PauseMenuScreen(game, this, playerName);
			game.setScreen(pm);
			dispose();
			setPauseMenu(true);
		...
		if (sSoldierList.size() < 7 && !isBossOnStage) {
	...
	public void loader(JFileChooser chooser) {
		...
		try {
			...
			lp.setScore((Integer) save.readObject());
			// Chiudo il file.
			save.close();
		} catch (Exception exc) {
			exc.printStackTrace(); // Se c'è stato un errore stampa info.
	...
	public void refreshLists() { // riempio la lista scorrendo i vari Identificatori
	...
	public void setPlayer(Personaggio lp) {
		this.lp = lp;
	}
